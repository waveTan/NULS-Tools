    ...
    <el-row class="header">
        <el-row class="pc w1200">
            ...
            <el-col class="user fr font14">
                ...
                <div class="fl">
                    <el-menu :default-active="rightActiveIndex" mode="horizontal" class="fl right-menu"
                             @select="handleSelect">
                        <el-submenu index="userList" v-if="accountList.length!==0" class="user-lists">
                            <template slot="title">
            ...
            <div class="address-info">
                <div class="ad tr">{{currentAccount.addresss}}<span v-show="currentAccount.alias">({{currentAccount.alias}})</span>
                </div>
                <div class="yue tr">可用：
                    <font v-if="!balanceLoading">{{currentAccount.balance}}</font>
...
<script>
    ...
    export default {
        ...
        components: {},
        watch: {
            "currentAccount.address": function (val, oldVal) {
                if (val && val !== oldVal) {
                    this.balanceLoading = true;
                    this.currentAccount.addresss = superLong(this.currentAccount.address, 6);
                    this.getAddressInfo(this.currentAccount.address);
                }
            }
        },
        methods: {
            ...
            async getAddressInfo(address) {
                ...
                if (addressInfo.success) {
                    this.currentAccount.addresss = superLong(addressInfo.data.address, 6);
                    this.currentAccount.alias = addressInfo.data.alias;
                    this.currentAccount.balance = tofix(divisionDecimals(addressInfo.data.balance), 3, -1);
