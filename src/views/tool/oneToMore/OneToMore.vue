  ...
  <div class="w1200 one_to" v-loading="oneToMoreLoading">
    ...
    <el-dialog :title="$t('oneTo.oneTo3')" :visible.sync="oneToMoreDialog" width="45rem" class="one_to_dialog">
      <div class="dialog_info">
        <ul>
          <li><span>{{$t('transfer.transfer0')}}</span><font>{{oneToMoreForm.fromAddress}}</font></li>
          ...
          <li class="cb"><span>{{$t('locking.locking91')}}</span><font>{{oneToMoreForm.fee}}<font class="fCN">
            NULS</font></font></li>
...
<script>
  ...
  import {getBalanceOrNonceByAddress, passwordVerification, validateAndBroadcast} from '@/api/requestData'
  ...
  export default {
    ...
    mounted() {
      //console.log(this.accontInfo);
      this.oneToMoreForm.fromAddress = this.accontInfo.address;
      this.getAccountBalance(chainInfo.chainId, chainInfo.assetId, this.accontInfo.address);
    ...
    methods: {
      ...
      async getAccountBalance(chainId, assetId, address) {
        let resData = await getBalanceOrNonceByAddress(address, chainId, assetId);
        //console.log(resData);
        if (resData.success) {
          this.accontInfo.balance = Number(divisionDecimals(resData.data.balance, 8));
        }
      },
      ...
      async toAddressChange(index) {
        if (20 < this.oneToMoreForm.toList[index].toAddress.length && this.oneToMoreForm.toList[index].toAddress.length < 60) {
          ...
          if (chainInfo.chainId !== addressInfo.chainId || chainInfo.assetsId !== addressInfo.type) {
            this.$message({
      ...
      async oneToMorePassSubmit(password) {
        ...
        let tAssemble = await nuls.transactionAssemble(inOrOutputs.data.inputs, inOrOutputs.data.outputs, htmlEncodeByRegExp(this.oneToMoreForm.remarks), 2);
        ...
        if (!validateTxhex.success) {
          ...
          this.$message({
            ...
            message: this.$t('tips.tips15') + " txHash:<a class='yellow'>" + validateTxhex.hash + "</a>",
            type: 'success',
            duration: '5000'
          });
        }
      ...
      async inputsOrOutputs(addressInfo, addressList, total) {
        let balanceInfo = await getBalanceOrNonceByAddress(addressInfo.address, chainInfo.chainId, chainInfo.assetsId);
        ...
        let inputs = [{
          ...
          amount: timesDecimals(total, 8).toString(),
          locked: 0,
        ...
        for (let item of addressList) {
          outputs.push({
            ...
            assetsChainId: chainInfo.chainId,
            assetsId: chainInfo.assetsId,
            amount: timesDecimals(item.amount, 8).toString(),
            lockTime: 0
          })
        }
